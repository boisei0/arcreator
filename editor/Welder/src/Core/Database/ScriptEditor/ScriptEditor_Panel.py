...
class ScriptEditor_Panel(Templates.ScriptEditor_Panel, PanelBase):
    ...
    _arc_panel_info_data = {"Name": "Script Editor Panel", "Caption": "Script Editor Panel", "CaptionV": True,
    ...
    def __init__(self, parent, index=-1):
        ...
        path = os.path.join(os.path.dirname(Kernel.GlobalObjects.get_value("CurrentProjectDir")), 'Data', 'Scripts')
        try:
            SM.LoadScripts()
        except:
    ...
    def CreateToolBar(self):
        ...
        self.toolBar.AddSimpleTool(6, art(wx.ART_FIND_AND_REPLACE), 'Find and Replace',
                                   'Open Find & Replace window for replacing text')
        self.toolBar.AddSeparator()
        self.toolBar.AddSimpleTool(7, art(wx.ART_HELP_SETTINGS), 'Settings', 'Opens the settings window')
        self.toolBar.AddSimpleTool(8, art(wx.ART_LIST_VIEW), 'Normalize Indents',
                                   'Applies conventional Ruby indenting to the script')
