...
class Actors_Panel(Templates.Actors_Panel, PanelBase):
    ...
    _arc_panel_info_data = {"Name": "Actors Panel", "Caption": "Actors Panel", "CaptionV": True,
    ...
    def refreshWeapons(self):
        ...
        for i in xrange(weaponSlots):
            ...
            for id_ in ids:
                if DataWeapons[id_] is None:
                    DataWeapons[id_] = RPG.Weapon()
            items.extend(
            ...
            if DM.ARC_FORMAT:
                ...
                id_, index = self.SelectedActor.weapon_id, 0
                if id_ != 0:
                    try:
                        index = self.GetWeaponIDs().index(id_) + 1
                    except ValueError:
    ...
    def glCanvasCharacter_DoubleClick(self, event):
        ...
        result = DM.ChooseGraphic(self, 'Characters', self.SelectedActor.character_name,
        ...
        if ctrlIndex == 0:
            ...
            if DM.ARC_FORMAT:
                ...
                if selection == 0:
                    ...
                    self.SelectedActor.weapon_id = self.GetWeaponIDs()[selection - 1]
        else:
            # Armor Changed
            if selection == 0:
