...
IF ERRORLEVEL 1 (
	...
	xcopy "2012 DLL\*Android.dll" "%MsBuildCppDir%\Android" /E

	if exist "%MsBuildCppDir%\Android-x86" (
		
		Echo.An Android-x86 Cpp MsBuild toolset already exists.
		Echo.Continuing will delete the version already installed to this directory:

		rd "%MsBuildCppDir%\Android-x86" /s
		pause
		if exist "%MsBuildCppDir%\Android-x86" (
			goto cleanup
		)
	...
	echo.Installing Android-x86 MSBuild files:
	cd /d %~dp0
	xcopy "Android-x86\*.*" "%MsBuildCppDir%\Android-x86" /E
	xcopy "2012 DLL\*Android-x86.dll" "%MsBuildCppDir%\Android-x86" /E
